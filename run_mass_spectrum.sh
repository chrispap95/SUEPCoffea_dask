...
if [ $signal -eq 1 ]; then
    echo "Processing signal for ${era}..."
    python runner.py \
        --workflow SUEP_coffea_mass_spectrum -o "processor_output_files/${tag}_${era}" \
...
if [ $background -eq 1 ]; then
    ...
    python runner.py \
        ...
        --executor futures -j "$(awk "BEGIN { print int($(nproc) * 0.33) }")" --chunk 30000
fi

if [ $data -eq 1 ]; then
    echo "Processing data for ${era}..."
    python runner.py \
        --workflow SUEP_coffea_mass_spectrum -o "processor_output_files/${tag}_${era}" --json $data_filelist \
        --era "$era" --executor futures -j "$(awk "BEGIN { print int($(nproc) * 0.33) }")" --chunk 30000
fi
