...
def muon_scale_factors(events, muons, era, is_data=False):
    if era == "2016APV":
        era = "2016a"
    elif era == "2016":
        era = "2016b"
    ...
    if is_data:
        ...
        muons["pt_SF"] = muons.pt * scale_DT
        muons["pt_SF_up"] = muons.pt * (scale_DT + scale_DT_error)
        muons["pt_SF_down"] = muons.pt * (scale_DT - scale_DT_error)
        return muons
    ...
    muons["pt_SF"] = muons.pt * mu_SF
    muons["pt_SF_up"] = muons.pt * (mu_SF + mu_SF_error)
    muons["pt_SF_down"] = muons.pt * (mu_SF - mu_SF_error)  # type: ignore[assignment]
