...
class SUEP_cluster(processor.ProcessorABC):
    ...
    def fill_preclustering_histograms(self, events, output):
        ...
        neutral_iso_cuts = 1.001 * np.logspace(-1, 1, 11)[:-1]
        pt_cuts = 1.001 * np.linspace(25, 50, 11)[:-1]
        ...
        for (
            ...
            neutral_iso_cut,
            pt_cut,
        ) in itertools.product(ip3d_cuts, mini_iso_cuts, neutral_iso_cuts, pt_cuts):
            cut = (
                (muons.pt < pt_cut)
                & (muons.ip3d < ip3d_cut)
                & (muons.miniPFRelIso_all < mini_iso_cut)
                & ((muons.miniPFRelIso_all - muons.miniPFRelIso_chg) < neutral_iso_cut)
            )
    ...
    def process(self, events):
        ...
        histograms = {
            "ip3d_cut_vs_mini_iso_cut_vs_neutral_iso_cut_vs_pt_cut_vs_nMuon": hist.Hist.new.Reg(
                10,
                1e-3,
                0.1,
                name="ip3d_cut",
            ...
            .Reg(
                ...
                0.1,
                10,
                name="neutral_iso_cut",
                label="neutral_iso_cut",
                transform=hist.axis.transform.log,
            )
            .Reg(10, 25, 50, name="pt_cut", label="pt_cut")
