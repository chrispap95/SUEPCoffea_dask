...
def parse_args():
    ...
    parser.add_argument(
        "--dest",
        type=str,
        default=str(pathlib.Path(__file__).parent / "QCD_HT_systematics_plots"),
        help="Destination directory to save the plots. Default is "
        f"{pathlib.Path(__file__).parent / 'QCD_HT_systematics_plots'}.",
    )
...
def plot_systematics(args, plots, sample, region):
    ...
    sample_tag = sample.replace(f"_{year}", "") + " - "
    ...
    for syst in sorted(systematics):
        ...
        if max(ratio_up) > 1.05 or min(ratio_down) < 0.95:
            ax2.set_ylim(0.9, 1.1)
        if max(ratio_up) > 1.1 or min(ratio_down) < 0.9:
            ax2.set_ylim(0.8, 1.2)
        if max(ratio_up) > 1.2 or min(ratio_down) < 0.8:
            ax2.set_ylim(0.5, 1.5)
        if max(ratio_up) > 1.5 or min(ratio_down) < 0.5:
        ...
        plt.savefig(
            os.path.join(
                ...
                args.tag,
                year,
                f"{region}_{sample}_{syst}_{year}.pdf",
            ),
            bbox_inches="tight",
        )
        plt.close()
