...
def calculate_k_factor(plots, year, region="CR_cb", process="QCD_Pt_MuEnrichedPt5"):
    ...
    tot_bkg = plots["DY_" + year][region][:, ::sum].copy().reset()
    for mc_proc in mc_processes:
        if process == mc_proc:
            continue
        tot_bkg += plots[f"{mc_proc}_{year}"][region][:, ::sum]
    k_factor = (
        plots["Data_" + year][region][:, ::sum].sum().value - tot_bkg.sum().value
    ) / plots[f"{process}_{year}"][region][:, ::sum].sum().value
    return k_factor
...
if "__main__" == __name__:
    ...
    if args.data and args.normalize:
        ...
        for year in track(years_to_load, description="Calculating k-factors"):
            k_factor_qcd[year] = calculate_k_factor(plots, year, region="CR_cb_muon_pt")
