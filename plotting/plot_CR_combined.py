...
def plot_CR_combined(args, plots):
    ...
    signal_labels = [
        r"$m_S=125\,$GeV, $m_\phi=8\,$GeV," + "\n" + r"$T=8\,$GeV, $m_{A'}=0.5\,$GeV",
        r"$m_S=125\,$GeV, $m_\phi=4\,$GeV," + "\n" + r"$T=16\,$GeV, $m_{A'}=0.5\,$GeV",
        r"$m_S=125\,$GeV, $m_\phi=8\,$GeV," + "\n" + r"$T=16\,$GeV, $m_{A'}=0.5\,$GeV",
        r"$m_S=125\,$GeV, $m_\phi=8\,$GeV," + "\n" + r"$T=32\,$GeV, $m_{A'}=0.5\,$GeV",
    ]
    ...
    if args.ratio:
        ...
        plt.subplots_adjust(bottom=0.17, top=0.92, left=0.1, right=0.95)
    ...
    ln_x_positions = [0.166, 0.549, 0.931]
    ln_y_range = [0.08, 0.72]
    lines = []
    for ln_x_pos in ln_x_positions:
        lines.append(
            Line2D(
                [ln_x_pos, ln_x_pos],
                ln_y_range,
    ...
    plt.xlabel(r"$n_{muon}$", fontsize=32)
    plt.sca(ax1)
    ...
    plt.ylabel("events")
    plt.tight_layout()
    plt.savefig(f"{args.dest}/CR_combined.pdf", bbox_inches="tight")
    plt.close()
...
if "__main__" == __name__:
    ...
    for sample in plots:
        ...
        h_comb["CR_DY bin 2"] = plots[sample]["CR_prompt"][3j]
        h_comb["CR_DY bin 3"] = plots[sample]["CR_prompt"][4j]
        h_comb["CR_DY bin 4"] = plots[sample]["CR_prompt"][5j]
